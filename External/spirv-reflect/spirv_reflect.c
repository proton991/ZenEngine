                                                        ...
                                                        size_t* res_size)
{
    *pp_res   = NULL;
    *res_size = 0;
    if (IsNull(p_arr0) || IsNull(p_arr1)) { return SPV_REFLECT_RESULT_SUCCESS; }
    ...
    {
        if (p_idx0->variable_ptr < *p_idx1) { ++p_idx0; }
        else if (p_idx0->variable_ptr > *p_idx1) { ++p_idx1; }
        else
        {
            ++*res_size;
            ++p_idx0;
    ...
    if (*res_size > 0)
    {
        *pp_res = (uint32_t*)calloc(*res_size, sizeof(**pp_res));
        if (IsNull(*pp_res)) { return SPV_REFLECT_RESULT_ERROR_ALLOC_FAILED; }
...
{
    bool in_range = false;
    if (IsNotNull(p_parser)) { in_range = (index < p_parser->spirv_word_count); }
    return in_range;
}
...
#define IF_READU32_CAST(result, parser, word_offset, cast_to_type, value)                          \
    ...
    {                                                                                              \
        ...
        if ((result) == SPV_REFLECT_RESULT_SUCCESS) { value = (cast_to_type)if_readu32_cast_u32; } \
    }
    ...
    const SpvReflectPrvDecorations* p_decoration_fields)
{
    ...
    if (p_decoration_fields->is_row_major) { decorations |= SPV_REFLECT_DECORATION_ROW_MAJOR; }
    if (p_decoration_fields->is_column_major)
    {
        decorations |= SPV_REFLECT_DECORATION_COLUMN_MAJOR;
    }
    if (p_decoration_fields->is_built_in) { decorations |= SPV_REFLECT_DECORATION_BUILT_IN; }
    if (p_decoration_fields->is_noperspective)
    {
        decorations |= SPV_REFLECT_DECORATION_NOPERSPECTIVE;
    }
    if (p_decoration_fields->is_flat) { decorations |= SPV_REFLECT_DECORATION_FLAT; }
    ...
    if (p_decoration_fields->is_patch) { decorations |= SPV_REFLECT_DECORATION_PATCH; }
    if (p_decoration_fields->is_per_vertex) { decorations |= SPV_REFLECT_DECORATION_PER_VERTEX; }
    if (p_decoration_fields->is_per_task) { decorations |= SPV_REFLECT_DECORATION_PER_TASK; }
...
static SpvReflectPrvAccessChain* FindAccessChain(SpvReflectPrvParser* p_parser, uint32_t id)
{
    ...
    {
        if (p_parser->access_chains[i].result_id == id) { return &p_parser->access_chains[i]; }
    }
    return 0;
}
...
{
    if (p_code == NULL) { return SPV_REFLECT_RESULT_ERROR_NULL_POINTER; }

    if (size < SPIRV_MINIMUM_FILE_SIZE) { return SPV_REFLECT_RESULT_ERROR_SPIRV_INVALID_CODE_SIZE; }
...
static void DestroyParser(SpvReflectPrvParser* p_parser)
{
    if (!IsNull(p_parser->nodes))
    {
        ...
        for (size_t i = 0; i < p_parser->node_count; ++i)
        {
            ...
            if (IsNotNull(p_node->member_names)) { SafeFree(p_node->member_names); }
            if (IsNotNull(p_node->member_decorations)) { SafeFree(p_node->member_decorations); }
        }
...
static SpvReflectResult ParseNodes(SpvReflectPrvParser* p_parser)
{
    ...
    while (spirv_word_index < p_parser->spirv_word_count)
    {
        ...
        if (node_word_count == 0) { return SPV_REFLECT_RESULT_ERROR_SPIRV_INVALID_INSTRUCTION; }
        if (op == SpvOpAccessChain) { ++(p_parser->access_chain_count); }
        spirv_word_index += node_word_count;
        ++node_count;
    }

    if (node_count == 0) { return SPV_REFLECT_RESULT_ERROR_SPIRV_UNEXPECTED_EOF; }

    // Allocate nodes
    p_parser->node_count = node_count;
    p_parser->nodes = (SpvReflectPrvNode*)calloc(p_parser->node_count, sizeof(*(p_parser->nodes)));
    if (IsNull(p_parser->nodes)) { return SPV_REFLECT_RESULT_ERROR_ALLOC_FAILED; }
    ...
    if (p_parser->access_chain_count > 0)
    {
        ...
        if (IsNull(p_parser->access_chains)) { return SPV_REFLECT_RESULT_ERROR_ALLOC_FAILED; }
    }
    ...
    while (spirv_word_index < p_parser->spirv_word_count)
    {
        ...
        switch (p_node->op)
        {
            ...
            case SpvOpSource:
            {
                ...
                if (p_node->word_count >= 5)
                {
                    ...
                    if (IsNull(p_source_temp)) { return SPV_REFLECT_RESULT_ERROR_ALLOC_FAILED; }
            ...
            case SpvOpSourceContinued:
            {
                ...
                if (IsNull(p_continued_source)) { return SPV_REFLECT_RESULT_ERROR_ALLOC_FAILED; }
                ...
                if (p_access_chain->index_count > 0)
                {
                         ...
                         ++index_index)
                    {
                        ...
                        CHECKED_READU32(p_parser,
                                        p_node->word_offset + SPIRV_ACCESS_CHAIN_INDEX_OFFSET +
                                            index_index,
                                        index_id);
        ...
        if (p_node->is_type) { ++(p_parser->type_count); }
...
static SpvReflectResult ParseStrings(SpvReflectPrvParser* p_parser)
{
    ...
    // Early out
    if (p_parser->string_count == 0) { return SPV_REFLECT_RESULT_SUCCESS; }
    ...
    if (IsNotNull(p_parser) && IsNotNull(p_parser->spirv_code) && IsNotNull(p_parser->nodes))
    {
        ...
        {
            SpvReflectPrvNode* p_node = &(p_parser->nodes[i]);
            if (p_node->op != SpvOpString) { continue; }
...
static SpvReflectResult ParseSource(SpvReflectPrvParser* p_parser, SpvReflectShaderModule* p_module)
{
    ...
    if (IsNotNull(p_parser) && IsNotNull(p_parser->spirv_code))
    {
        ...
        if (IsNotNull(p_parser->source_embedded))
        {
            ...
            if (IsNull(p_source)) { return SPV_REFLECT_RESULT_ERROR_ALLOC_FAILED; }
                                      ...
                                      size_t first_label_index)
{
    ...
    {
        SpvReflectPrvNode* p_node = &(p_parser->nodes[i]);
        if (p_node->op == SpvOpFunctionEnd) { break; }
        switch (p_node->op)
        {
    ...
    if (p_func->callee_count > 0)
    {
        ...
        if (IsNull(p_func->callees)) { return SPV_REFLECT_RESULT_ERROR_ALLOC_FAILED; }
    }
    ...
    if (p_func->accessed_variable_count > 0)
    {
        ...
        if (IsNull(p_func->accessed_variables)) { return SPV_REFLECT_RESULT_ERROR_ALLOC_FAILED; }
    }
...
static SpvReflectResult ParseFunctions(SpvReflectPrvParser* p_parser)
{
    ...
    if (IsNotNull(p_parser) && IsNotNull(p_parser->spirv_code) && IsNotNull(p_parser->nodes))
    {
        ...
        if (IsNull(p_parser->functions)) { return SPV_REFLECT_RESULT_ERROR_ALLOC_FAILED; }

        size_t function_index = 0;
        for (size_t i = 0; i < p_parser->node_count; ++i)
        {
            SpvReflectPrvNode* p_node = &(p_parser->nodes[i]);
            if (p_node->op != SpvOpFunction) { continue; }
            ...
            for (; i < p_parser->node_count; ++i)
            {
                ...
                if (p_parser->nodes[i].op == SpvOpFunctionEnd) { break; }
            }
            if (!func_definition) { continue; }
        ...
        for (size_t i = 0; i < p_parser->function_count; ++i)
        {
            ...
            if (p_func->callee_count == 0) { continue; }
...
static SpvReflectResult ParseMemberCounts(SpvReflectPrvParser* p_parser)
{
    ...
    if (IsNotNull(p_parser) && IsNotNull(p_parser->spirv_code) && IsNotNull(p_parser->nodes))
    {
        for (size_t i = 0; i < p_parser->node_count; ++i)
        {
            ...
            if (IsNull(p_target_node)) { continue; }

            if (member_index == INVALID_VALUE) { return SPV_REFLECT_RESULT_ERROR_RANGE_EXCEEDED; }
        ...
        {
            SpvReflectPrvNode* p_node = &(p_parser->nodes[i]);
            if (p_node->member_count == 0) { continue; }

            p_node->member_names =
                (const char**)calloc(p_node->member_count, sizeof(*(p_node->member_names)));
            if (IsNull(p_node->member_names)) { return SPV_REFLECT_RESULT_ERROR_ALLOC_FAILED; }
...
static SpvReflectResult ParseNames(SpvReflectPrvParser* p_parser)
{
    ...
    if (IsNotNull(p_parser) && IsNotNull(p_parser->spirv_code) && IsNotNull(p_parser->nodes))
    {
        ...
        {
            SpvReflectPrvNode* p_node = &(p_parser->nodes[i]);
            if ((p_node->op != SpvOpName) && (p_node->op != SpvOpMemberName)) { continue; }
...
static SpvReflectResult ParseDecorations(SpvReflectPrvParser* p_parser)
{
    for (uint32_t i = 0; i < p_parser->node_count; ++i)
    {
        ...
        if (p_node->op == SpvOpMemberDecorate) { member_offset = 1; }
        ...
        switch (decoration)
        {
            ...
            case SpvDecorationSpecId:
            {
                uint32_t word_offset = p_node->word_offset + member_offset + 3;
                CHECKED_READU32(p_parser, word_offset,
                                p_target_decorations->specialization_constant.value);
                                             ...
                                             uint32_t** pp_uniforms)
{
    *p_uniform_count = p_module->descriptor_binding_count;
    if (*p_uniform_count == 0) { return SPV_REFLECT_RESULT_SUCCESS; }
    *pp_uniforms = (uint32_t*)calloc(*p_uniform_count, sizeof(**pp_uniforms));

    if (IsNull(*pp_uniforms)) { return SPV_REFLECT_RESULT_ERROR_ALLOC_FAILED; }
                                  ...
                                  SpvReflectTypeDescription* p_type)
{
    ...
    if (p_node->member_count > 0)
    {
        ...
        else { result = SPV_REFLECT_RESULT_ERROR_ALLOC_FAILED; }
    }
    ...
    if (result == SPV_REFLECT_RESULT_SUCCESS)
    {
        ...
        switch (p_node->op)
        {
            ...
            case SpvOpTypeImage:
            {
                ...
                else { result = SPV_REFLECT_RESULT_ERROR_SPIRV_INVALID_ID_REFERENCE; }
            ...
            case SpvOpTypeStruct:
            {
                ...
                for (; word_index < p_node->word_count; ++word_index, ++member_index)
                {
                    ...
                    result = ParseType(p_parser, p_member_node, p_member_decorations, p_module,
                                       p_member_type);
                    if (result != SPV_REFLECT_RESULT_SUCCESS) { break; }
                    // This looks wrong
        ...
        if (result == SPV_REFLECT_RESULT_SUCCESS)
        {
            ...
            if (IsNull(p_type->type_name)) { p_type->type_name = p_node->name; }
        }
    }
...
static SpvReflectResult ParseTypes(SpvReflectPrvParser* p_parser, SpvReflectShaderModule* p_module)
{
    ...
    if (p_parser->physical_pointer_struct_count > 0)
    {
        p_parser->physical_pointer_structs = (SpvReflectPrvPhysicalPointerStruct*)calloc(
            p_parser->physical_pointer_struct_count, sizeof(*(p_parser->physical_pointer_structs)));
...
static SpvReflectResult ParseCapabilities(SpvReflectPrvParser* p_parser,
                                          SpvReflectShaderModule* p_module)
{
    if (p_parser->capability_count == 0) { return SPV_REFLECT_RESULT_SUCCESS; }
    ...
    if (IsNull(p_module->capabilities)) { return SPV_REFLECT_RESULT_ERROR_ALLOC_FAILED; }
    ...
    {
        SpvReflectPrvNode* p_node = &(p_parser->nodes[i]);
        if (SpvOpCapability != p_node->op) { continue; }
                                                ...
                                                SpvReflectShaderModule* p_module)
{
    ...
    if (p_module->descriptor_binding_count == 0) { return SPV_REFLECT_RESULT_SUCCESS; }
    ...
    if (IsNull(p_module->descriptor_bindings)) { return SPV_REFLECT_RESULT_ERROR_ALLOC_FAILED; }
    ...
    for (size_t i = 0; i < p_parser->node_count; ++i)
    {
        ...
        if (IsNull(p_type)) { return SPV_REFLECT_RESULT_ERROR_SPIRV_INVALID_ID_REFERENCE; }
        ...
        if (p_type->op == SpvOpTypePointer)
        {
            ...
            if (IsNull(p_type_node)) { return SPV_REFLECT_RESULT_ERROR_SPIRV_INVALID_ID_REFERENCE; }
        ...
        if ((int)p_descriptor->descriptor_type == (int)INVALID_VALUE)
        {
            switch (p_type->type_flags & SPV_REFLECT_TYPE_FLAG_EXTERNAL_MASK)
            {
                ...
                case SPV_REFLECT_TYPE_FLAG_EXTERNAL_BLOCK:
                {
                    ...
                    else { assert(false && "unknown struct"); }
                }
                break;
                                                     ...
                                                     SpvReflectBlockVariable* p_var)
{
    ...
    if (IsNotNull(p_type->members) && (p_type->member_count > 0))
    {
        ...
        if (IsNull(p_var->members)) { return SPV_REFLECT_RESULT_ERROR_ALLOC_FAILED; }
        ...
        for (uint32_t member_index = 0; member_index < p_type->member_count; ++member_index)
        {
                ...
                (p_member_type->type_flags & SPV_REFLECT_TYPE_FLAG_REF))
            {
                ...
                {
                    uint32_t struct_id =
                        FindType(p_module, p_member_type->id)->struct_type_description->id;
    ...
    if (has_non_writable) { p_var->decoration_flags |= SPV_REFLECT_DECORATION_NON_WRITABLE; }
...
static SpvReflectResult ParseDescriptorBlockVariableSizes(SpvReflectPrvParser* p_parser,
                                                          ...
                                                          SpvReflectBlockVariable* p_var)
{
    if (p_var->member_count == 0) { return SPV_REFLECT_RESULT_SUCCESS; }
    ...
    {
        SpvReflectBlockVariable* p_member_var = &p_var->members[member_index];
        if (is_parent_root) { p_member_var->absolute_offset = p_member_var->offset; }
        else
        {
    ...
    for (uint32_t member_index = 0; member_index < (p_var->member_count - 1); ++member_index)
    {
        ...
        if (is_parent_rta) { p_member_var->padded_size = p_member_var->size; }
    }
                                                          ...
                                                          SpvReflectBlockVariable* p_var)
{
    ...
    if (override_op_type != (SpvOp)INVALID_VALUE) { op_type = override_op_type; }
    ...
    switch (op_type)
    {
        ...
        case SpvOpTypeArray:
        {
            ...
            while ((p_type->op == SpvOpTypeArray) && (index_index < p_access_chain->index_count))
            {
                ...
                if (p_type == NULL) { return SPV_REFLECT_RESULT_ERROR_SPIRV_INVALID_ID_REFERENCE; }
        ...
        case SpvOpTypeStruct:
        {
            ...
            if (p_access_chain->index_count == 0) { return SPV_REFLECT_RESULT_SUCCESS; }
                                              ...
                                              SpvReflectShaderModule* p_module)
{
         ...
         ++descriptor_index)
    {
             ...
             ++access_chain_index)
        {
            ...
            if (p_descriptor->spirv_id != p_access_chain->base_id) { continue; }
    ...
    bool* p_has_built_in)
{
    ...
    if (p_type->op == SpvOpTypeArray) { ApplyArrayTraits(p_type, &p_var->array); }
                                                ...
                                                uint32_t* p_interface_variable_ids)
{
    ...
    for (size_t i = 0; i < interface_variable_count; ++i)
    {
        ...
        if (IsNull(p_node)) { return SPV_REFLECT_RESULT_ERROR_SPIRV_INVALID_ID_REFERENCE; }

        if (p_node->storage_class == SpvStorageClassInput) { p_entry->input_variable_count += 1; }
    ...
    if (p_entry->input_variable_count > 0)
    {
        ...
        if (IsNull(p_entry->input_variables)) { return SPV_REFLECT_RESULT_ERROR_ALLOC_FAILED; }
    }
    ...
    if (p_entry->output_variable_count > 0)
    {
        ...
        if (IsNull(p_entry->output_variables)) { return SPV_REFLECT_RESULT_ERROR_ALLOC_FAILED; }
    }
    ...
    if (p_entry->interface_variable_count > 0)
    {
        ...
        if (IsNull(p_entry->interface_variables)) { return SPV_REFLECT_RESULT_ERROR_ALLOC_FAILED; }
    }
    ...
    for (size_t i = 0; i < interface_variable_count; ++i)
    {
        ...
        SpvReflectResult result =
            ParseInterfaceVariable(p_parser, &p_node->decorations, &p_type_node->decorations,
                                   p_module, p_type, p_var, &has_built_in);
        if (result != SPV_REFLECT_RESULT_SUCCESS)
        {
            SPV_REFLECT_ASSERT(false);
            return result;
        ...
        if (has_built_in) { p_var->decoration_flags |= SPV_REFLECT_DECORATION_BUILT_IN; }
        ...
        // Built in
        if (p_node->decorations.is_built_in) { p_var->built_in = p_node->decorations.built_in; }
    }
                                                  ...
                                                  uint32_t** p_push_constants)
{
    *p_push_constant_count = p_module->push_constant_block_count;
    if (*p_push_constant_count == 0) { return SPV_REFLECT_RESULT_SUCCESS; }
    *p_push_constants = (uint32_t*)calloc(*p_push_constant_count, sizeof(**p_push_constants));

    if (IsNull(*p_push_constants)) { return SPV_REFLECT_RESULT_ERROR_ALLOC_FAILED; }
                                          ...
                                          uint32_t depth)
{
    ...
    if (IsNotNull(p_func_ids)) { p_func_ids[(*p_func_count)++] = p_func->id; }
    else { ++*p_func_count; }
                                                     ...
                                                     uint32_t* push_constants)
{
    ...
    if (p_func == NULL) { return SPV_REFLECT_RESULT_ERROR_SPIRV_INVALID_ID_REFERENCE; }
    ...
    if (called_function_count > 0)
    {
        ...
        if (IsNull(p_called_functions)) { return SPV_REFLECT_RESULT_ERROR_ALLOC_FAILED; }
    }
    ...
    SpvReflectResult result0  = IntersectSortedAccessedVariable(
        p_used_accesses, used_acessed_count, uniforms, uniform_count, &p_entry->used_uniforms,
        &used_uniform_count);

    size_t used_push_constant_count = 0;
    SpvReflectResult result1        = IntersectSortedAccessedVariable(
    ...
    for (uint32_t i = 0; i < p_module->descriptor_binding_count; ++i)
    {
        ...
        {
            if (p_used_accesses[j].variable_ptr == p_binding->spirv_id) { p_binding->accessed = 1; }
        }
    }

    SafeFree(p_used_accesses);
    if (result0 != SPV_REFLECT_RESULT_SUCCESS) { return result0; }
    if (result1 != SPV_REFLECT_RESULT_SUCCESS) { return result1; }
                                         ...
                                         SpvReflectShaderModule* p_module)
{
    ...
    if (IsNull(p_module->entry_points)) { return SPV_REFLECT_RESULT_ERROR_ALLOC_FAILED; }
    ...
    {
        SpvReflectPrvNode* p_node = &(p_parser->nodes[i]);
        if (p_node->op != SpvOpEntryPoint) { continue; }
        ...
        if (interface_variable_count > 0)
        {
            ...
            if (IsNull(p_interface_variables)) { return SPV_REFLECT_RESULT_ERROR_ALLOC_FAILED; }
        }
                                            ...
                                            SpvReflectShaderModule* p_module)
{
    ...
    if (IsNotNull(p_parser) && IsNotNull(p_parser->spirv_code) && IsNotNull(p_parser->nodes))
    {
        ...
        if (IsNull(indices)) { return SPV_REFLECT_RESULT_ERROR_ALLOC_FAILED; }
        for (size_t entry_point_idx = 0; entry_point_idx < p_module->entry_point_count;
             ++entry_point_idx)
        {
            ...
            if (p_entry_point->execution_mode_count > 0)
            {
                p_entry_point->execution_modes = (SpvExecutionMode*)calloc(
                    p_entry_point->execution_mode_count, sizeof(*p_entry_point->execution_modes));
        ...
        {
            SpvReflectPrvNode* p_node = &(p_parser->nodes[node_idx]);
            if (p_node->op != SpvOpExecutionMode) { continue; }
...
// -- GODOT begin --
static SpvReflectResult ParseSpecializationConstants(SpvReflectPrvParser* p_parser,
                                                     SpvReflectShaderModule* p_module)
{
    ...
    {
        SpvReflectPrvNode* p_node = &(p_parser->nodes[i]);
        if (p_node->op == SpvOpSpecConstantTrue || p_node->op == SpvOpSpecConstantFalse ||
            p_node->op == SpvOpSpecConstant)
        {
            p_module->specialization_constant_count++;
        }
    }

    if (p_module->specialization_constant_count == 0) { return SPV_REFLECT_RESULT_SUCCESS; }

    p_module->specialization_constants = (SpvReflectSpecializationConstant*)calloc(
        p_module->specialization_constant_count, sizeof(SpvReflectSpecializationConstant));
    ...
    for (size_t i = 0; i < p_parser->node_count; ++i)
    {
        ...
        switch (p_node->op)
        {
            ...
            case SpvOpSpecConstantTrue:
            {
                p_module->specialization_constants[index].constant_type =
            ...
            case SpvOpSpecConstant:
            {
                ...
                else { return SPV_REFLECT_RESULT_ERROR_PARSE_FAILED; }

                p_module->specialization_constants[index].default_value.int_bool_value =
                    default_value; //bits are the same for int and float
            }
            break;
        }

        p_module->specialization_constants[index].name = p_node->name;
        p_module->specialization_constants[index].constant_id =
            p_node->decorations.specialization_constant.value;
                                                ...
                                                SpvReflectShaderModule* p_module)
{
    ...
    if (p_module->push_constant_block_count == 0) { return SPV_REFLECT_RESULT_SUCCESS; }
    ...
    if (IsNull(p_module->push_constant_blocks)) { return SPV_REFLECT_RESULT_ERROR_ALLOC_FAILED; }
    ...
    for (size_t i = 0; i < p_parser->node_count; ++i)
    {
             ...
             ++access_chain_index)
        {
            ...
            if (p_push_constant->spirv_id != FindBaseId(p_parser, p_access_chain)) { continue; }
            SpvReflectBlockVariable* p_var =
        ...
        result = ParseDescriptorBlockVariableSizes(p_parser, p_module, true, false, false,
                                                   p_push_constant);
        if (result != SPV_REFLECT_RESULT_SUCCESS) { return result; }
...
static SpvReflectResult ParseEntrypointDescriptorSets(SpvReflectShaderModule* p_module)
{
    ...
    for (uint32_t i = 0; i < p_module->entry_point_count; ++i)
    {
        ...
        if (p_entry->descriptor_set_count > 0)
        {
            ...
            if (IsNull(p_entry->descriptor_sets)) { return SPV_REFLECT_RESULT_ERROR_ALLOC_FAILED; }
        }
        p_entry->descriptor_set_count = 0;
        for (uint32_t j = 0; j < p_module->descriptor_set_count; ++j)
        {
            ...
            if (IsNull(p_entry_set->bindings)) { return SPV_REFLECT_RESULT_ERROR_ALLOC_FAILED; }
            for (uint32_t k = 0; k < p_set->binding_count; ++k)
            {
                bool found = SearchSortedUint32(p_entry->used_uniforms, p_entry->used_uniform_count,
                                                p_set->bindings[k]->spirv_id);
...
static SpvReflectResult ParseDescriptorSets(SpvReflectShaderModule* p_module)
{
    ...
    for (uint32_t i = 0; i < p_module->descriptor_binding_count; ++i)
    {
        ...
        if (IsNull(p_target_set)) { return SPV_REFLECT_RESULT_ERROR_INTERNAL_ERROR; }
    ...
    for (uint32_t i = 0; i < SPV_REFLECT_MAX_DESCRIPTOR_SETS; ++i)
    {
        ...
        if (p_set->set != (uint32_t)INVALID_VALUE) { p_module->descriptor_set_count += 1; }
    }
    ...
    for (uint32_t i = 0; i < p_module->descriptor_set_count; ++i)
    {
        ...
        p_set->bindings =
            (SpvReflectDescriptorBinding**)calloc(p_set->binding_count, sizeof(*(p_set->bindings)));
                                           ...
                                           SpvReflectShaderModule* p_module)
{
...
#endif
    if (IsNull(p_module->_internal)) { return SPV_REFLECT_RESULT_ERROR_ALLOC_FAILED; }
    // Copy flags
    ...
    if (flags & SPV_REFLECT_MODULE_FLAG_NO_COPY)
    {
        ...
        p_module->_internal->spirv_code =
            const_cast<uint32_t*>(static_cast<const uint32_t*>(p_code)); // cast that const away
#else
    ...
    if (result != SPV_REFLECT_RESULT_SUCCESS) { spvReflectDestroyShaderModule(p_module); }
...
{
    if (IsNull(p_type) || p_type->copied) { return; }
...
{
    if (IsNull(p_block)) { return; }

    // We share pointers to Physical Pointer structs and don't want to double free
    if (p_block->flags & SPV_REFLECT_VARIABLE_FLAGS_PHYSICAL_POINTER_COPY) { return; }
...
{
    if (IsNull(p_module->_internal)) { return; }
    ...
    {
        SpvReflectTypeDescription* p_type = &p_module->_internal->type_descriptions[i];
        if (IsNotNull(p_type->members)) { SafeFreeTypes(p_type); }
        SafeFree(p_type->members);
    }
...
{
    if (IsNull(p_module)) { return NULL; }
...
{
    if (IsNull(p_module) || IsNull(entry_point)) { return NULL; }
...
{
    if (IsNull(p_module)) { return SPV_REFLECT_RESULT_ERROR_NULL_POINTER; }
    if (IsNull(p_count)) { return SPV_REFLECT_RESULT_ERROR_NULL_POINTER; }
    ...
    else { *p_count = p_module->descriptor_binding_count; }
    ...
    SpvReflectDescriptorBinding** pp_bindings)
{
    ...
    if (IsNull(p_entry)) { return SPV_REFLECT_RESULT_ERROR_ELEMENT_NOT_FOUND; }
    ...
    for (uint32_t i = 0; i < p_module->descriptor_binding_count; ++i)
    {
        ...
        if (found)
        {
            if (IsNotNull(pp_bindings))
            {
                if (count >= *p_count) { return SPV_REFLECT_RESULT_ERROR_COUNT_MISMATCH; }
                pp_bindings[count++] =
                                                   ...
                                                   SpvReflectDescriptorSet** pp_sets)
{
    ...
    else { *p_count = p_module->descriptor_set_count; }
                                                             ...
                                                             SpvReflectDescriptorSet** pp_sets)
{
    ...
    else { *p_count = p_entry->descriptor_set_count; }
                                                       ...
                                                       SpvReflectInterfaceVariable** pp_variables)
{
    ...
    else { *p_count = p_module->interface_variable_count; }
    ...
    SpvReflectInterfaceVariable** pp_variables)
{
    ...
    else { *p_count = p_entry->interface_variable_count; }
    ...
    SpvReflectSpecializationConstant** pp_constants)
{
    ...
    else { *p_count = p_module->specialization_constant_count; }
                                                   ...
                                                   SpvReflectInterfaceVariable** pp_variables)
{
    ...
    else { *p_count = p_module->input_variable_count; }
    ...
    SpvReflectInterfaceVariable** pp_variables)
{
    ...
    else { *p_count = p_entry->input_variable_count; }
                                                    ...
                                                    SpvReflectInterfaceVariable** pp_variables)
{
    ...
    else { *p_count = p_module->output_variable_count; }
    ...
    SpvReflectInterfaceVariable** pp_variables)
{
    ...
    else { *p_count = p_entry->output_variable_count; }
                                                       ...
                                                       SpvReflectBlockVariable** pp_blocks)
{
    ...
    else { *p_count = p_module->push_constant_block_count; }
    ...
    SpvReflectResult* p_result)
{
    ...
    if (IsNull(p_entry))
    {
        if (IsNotNull(p_result)) { *p_result = SPV_REFLECT_RESULT_ERROR_ELEMENT_NOT_FOUND; }
        return NULL;
    }
    const SpvReflectDescriptorBinding* p_descriptor = NULL;
    if (IsNotNull(p_module))
                                                          ...
                                                          SpvReflectResult* p_result)
{
    ...
    if (IsNotNull(p_module))
    {
        for (uint32_t index = 0; index < p_module->descriptor_set_count; ++index)
        {
            ...
            if (p_potential->set == set_number) { p_set = p_potential; }
        }
    }
    if (IsNotNull(p_result))
    {
    ...
    SpvReflectResult* p_result)
{
    ...
    if (IsNotNull(p_module))
    {
        for (uint32_t index = 0; index < p_module->input_variable_count; ++index)
        {
            ...
            if (p_potential->location == location) { p_var = p_potential; }
        }
    }
    if (IsNotNull(p_result))
    {
    ...
    SpvReflectResult* p_result)
{
    if (IsNull(semantic))
    {
        if (IsNotNull(p_result)) { *p_result = SPV_REFLECT_RESULT_ERROR_NULL_POINTER; }
        return NULL;
    }
    if (semantic[0] == '\0')
    {
...
{
    if (IsNull(p_module)) { return SPV_REFLECT_RESULT_ERROR_NULL_POINTER; }
    if (IsNull(p_binding)) { return SPV_REFLECT_RESULT_ERROR_NULL_POINTER; }
...
{
    if (IsNull(p_module)) { return SPV_REFLECT_RESULT_ERROR_NULL_POINTER; }
    if (IsNull(p_set)) { return SPV_REFLECT_RESULT_ERROR_NULL_POINTER; }
    SpvReflectDescriptorSet* p_target_set = NULL;
...
{
    if (IsNull(p_module)) { return SPV_REFLECT_RESULT_ERROR_NULL_POINTER; }
    if (IsNull(p_input_variable)) { return SPV_REFLECT_RESULT_ERROR_NULL_POINTER; }
    for (uint32_t index = 0; index < p_module->input_variable_count; ++index)
    {
...
{
    if (IsNull(p_module)) { return SPV_REFLECT_RESULT_ERROR_NULL_POINTER; }
    if (IsNull(p_output_variable)) { return SPV_REFLECT_RESULT_ERROR_NULL_POINTER; }
    for (uint32_t index = 0; index < p_module->output_variable_count; ++index)
    {
