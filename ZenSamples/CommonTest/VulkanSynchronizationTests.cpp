...
namespace zen
{
struct VulkanQueueTestAccess
{
    static void SetFenceManager(VulkanDevice& device, VulkanFenceManager* manager)
    {
        device.m_pFenceManager = manager;
    }
};
} // namespace zen
...
TEST(VulkanNameTests, InternedLayerNamesPreserveGlobalEnumerationAndCallerStorage)
{
    ...
    ScopedVulkanFunction<PFN_vkEnumerateInstanceExtensionProperties> enumerateExtensions(
...
protected:
    template <typename Callback> void WithQueue(Callback callback)
    {
        ...
        VulkanFenceManager manager(&device);
        VulkanQueueTestAccess::SetFenceManager(device, &manager);
...
TEST_P(VulkanQueueWaitTest, StopsAtRequestedSerialAndDoesNotWaitDuringPolling)
{
    WithQueue([](zen::VulkanQueue& queue) {
        EXPECT_TRUE(queue.WaitForSubmission(0, 0));
        EXPECT_FALSE(queue.WaitForSubmission(4, UINT64_MAX));
        EXPECT_TRUE(QueueDriver::waitedSerials.empty());
        EXPECT_FALSE(queue.WaitForSubmission(1, 0));
        EXPECT_TRUE(QueueDriver::waitedSerials.empty());
        ...
        EXPECT_TRUE(queue.WaitForSubmission(1, 0));
...
TEST_P(VulkanQueueWaitTest, RejectedSubmissionDoesNotPublishSerialOrCompletionAndCanBeRebuilt)
{
    WithQueue([timeline = GetParam()](zen::VulkanQueue& queue) {
        ...
        EXPECT_EQ(queue.GetLastSubmittedSerial(), 3u);
        EXPECT_EQ(queue.GetLastCompletedSerial(), 0u);
        EXPECT_FALSE(queue.WaitForSubmission(4, 0));
...
TEST_P(VulkanQueueWaitTest, FailedBatchRetainsOnlyItsAcceptedPrefix)
{
    WithQueue([timeline = GetParam()](zen::VulkanQueue& queue) {
        ...
        EXPECT_TRUE(queue.WaitForSubmission(queue.GetLastSubmittedSerial(), UINT64_MAX));
        EXPECT_FALSE(queue.WaitForSubmission(queue.GetLastSubmittedSerial() + 1, 0));
    });
}
