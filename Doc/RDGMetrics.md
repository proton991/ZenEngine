...
Queue captures use `RHICommandContextType` and the captured `RHIQueueCapabilities`. They derive semaphore classification from the live predecessor lists and native queue equivalence; separate semaphore-only predecessor arrays are no longer stored. The reported wait-stage mask comes from `RHISubmissionDependency::kWaitStage` (`ALL_COMMANDS`), which the Vulkan backend also uses.
