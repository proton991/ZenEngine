...
Texture and texture-view creation return null after logging native allocation/creation failures. Invalid view descriptions also return null before calling Vulkan. Callers must check the result before recording resource use. Failed texture creation releases the image and any views already created; failed additional-view creation preserves the existing texture and views so it can be retried. Vulkan render-target textures prepare their implicit attachment view before publishing the texture, keeping recoverable allocation failures at the resource-creation boundary.
