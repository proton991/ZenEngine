...
RenderDevice owns the last submission using each live physical resource and attaches waits for resources whose previous use was on another queue, including earlier reads before an overwrite. RenderGraph and RDGExecutor do not query backend queue progress or manage submission dependencies. RenderDevice updates submission history only after acceptance; rejection preserves the previous history. Queued graphics frames retain a scheduled dependency resolved when a later consumer reaches RHI; this can conservatively include other preceding graphics work. Dependencies are coalesced per producer queue for the whole consumer submission. Graphs with no shared resources do not wait for unrelated uploads. Explicit external-state invalidation still requires the caller to arrange completion/visibility of external work.

On timeline-capable devices, upload/transfer flush returns after native submission without waiting for GPU completion. Staging blocks and destination references retire against their completion serials. Pool pressure, frame-slot reuse, and shutdown can still wait. CPU staging copies and immediate native submission through `RHIThread::Invoke` remain synchronous. Transfer-incompatible work, including mipmap blits and transitions involving graphics-only stages, continues on the graphics queue; timeline-disabled devices retain the synchronous transfer fallback. Automatic async-compute scheduling is not implemented.
