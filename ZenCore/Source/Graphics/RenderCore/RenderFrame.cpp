...
namespace zen
{
val::CommandPool* RenderFrame::GetCommandPool(uint32_t                    queueFamilyIndex,
                                              val::CommandPool::ResetMode resetMode)
{
    auto it = m_cmdPools.find(queueFamilyIndex);
    if (it != m_cmdPools.end()) { return it->second.Get(); }
    ...
    auto [insertIt, inserted] =
        m_cmdPools.emplace(queueFamilyIndex, MakeUnique<val::CommandPool>(m_valDevice, cmdPoolCI));
...
void RenderFrame::Reset()
{
    ...
    for (auto& it : m_cmdPools) { it.second->ResetPool(); }
