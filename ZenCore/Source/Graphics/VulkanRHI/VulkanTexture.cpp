...
void VulkanRHI::GenerateTextureMipmaps(TextureHandle textureHandle, RHICommandList* cmdList)
{
    ...
    VulkanCommandBuffer* cmdBuffer   = vulkanCmdList->m_cmdBuffer;
    ...
    VkImageLayout originLayout = GetImageCurrentLayout(vkImage);
    // Transition first mip level to transfer source for read during blit
    ChangeImageLayout(cmdBuffer, vkImage, originLayout, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL,
    ...
    for (uint32_t i = 1; i < mipLevels; i++)
    {
        ...
        ChangeImageLayout(cmdBuffer, vkImage, GetImageCurrentLayout(vkImage),
                          VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, mipSubRange);

        // Blit from previous level
        vkCmdBlitImage(cmdBuffer->GetVkHandle(), vkImage, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL,
    ...
    ChangeImageLayout(cmdBuffer, vkImage, GetImageCurrentLayout(vkImage), originLayout,
                      VulkanTexture::GetSubresourceRange(VK_IMAGE_ASPECT_COLOR_BIT, 0, mipLevels, 0,
                                                         texture->imageCI.arrayLayers));
}
