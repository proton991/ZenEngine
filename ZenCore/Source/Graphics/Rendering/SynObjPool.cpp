...
VkFence SynObjPool::RequestFence()
{
    ...
    CHECK_VK_ERROR_AND_THROW(vkCreateFence(m_valDevice.GetHandle(), &fenceCI, nullptr, &fence), "Failed to create fence");
...
void SynObjPool::WaitForFences(uint32_t timeout) const
{
    ...
    CHECK_VK_ERROR_AND_THROW(vkWaitForFences(m_valDevice.GetHandle(), util::ToU32(m_fences.size()), m_fences.data(), true, timeout), "Failed to wait for fences");
}
...
void SynObjPool::ResetFences()
{
    ...
    CHECK_VK_ERROR_AND_THROW(vkResetFences(m_valDevice.GetHandle(), util::ToU32(m_fences.size()), m_fences.data()), "Failed to reset fences");
    m_numActiveFences = 0;
}
